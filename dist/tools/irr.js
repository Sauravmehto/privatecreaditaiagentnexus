...
exports.calculateIrrValue = calculateIrrValue;
exports.solveIrrBisection = solveIrrBisection;
...
exports.get_outstanding_at_breach = get_outstanding_at_breach;
exports.calculate_dpo_breakeven = calculate_dpo_breakeven;
exports.generate_warrant_irr = generate_warrant_irr;
exports.calculate_warrant_irr = calculate_warrant_irr;
exports.calculate_conv_note_irr = calculate_conv_note_irr;
...
function solveIrrBisection(cashFlows, lower = -0.95, upper = 5, tolerance = 1e-8, maxIter = 400) {
    let lo = lower;
    let hi = upper;
...
async function calculate_irr(input) {
...
async function calculate_term_loan_irr(input) {
...
async function calculate_note_irr(input) {
...
async function run_restructure_scenario(input) {
    const deal = await (0, db_1.findDealById)(input.deal_id);
    if (!deal) {
        throw new Error(`Deal not found: ${input.deal_id}`);
    }
...
async function compare_before_after_irr(input) {
...
async function compare_all_scenarios(input) {
    const scenarios = [
...
async function get_outstanding_at_breach(input) {
    const deal = await (0, db_1.findDealById)(input.deal_id);
    if (!deal)
        throw new Error(`Deal not found: ${input.deal_id}`);
    const schedule = buildSchedule({
...
async function calculate_dpo_breakeven(input) {
    const deal = await (0, db_1.findDealById)(input.deal_id);
    if (!deal)
        throw new Error(`Deal not found: ${input.deal_id}`);
    const base = {
    ...
    for (let i = 0; i < 80; i += 1) {
        ...
        for (let m = 1; m <= month; m += 1)
            flows.push(schedule[m - 1]?.total_payment ?? 0);
...
function blackScholesCall(spot, strike, volatility, years, riskFree) {
    if (spot <= 0 || strike <= 0 || volatility <= 0 || years <= 0)
        return 0;
    const d1 = (Math.log(spot / strike) + (riskFree + (volatility * volatility) / 2) * years) /
...
async function generate_warrant_irr(input) {
    const spot = input.fd_shares * input.coverage;
    const blackScholesFmv = blackScholesCall(spot, input.strike_val, 0.45, 3, 0.04);
    const scenarioPayoffs = input.scenarios.map((exitValue) => Math.max(0, exitValue * input.coverage - input.strike_val));
    const weightedPayoff = scenarioPayoffs.reduce((acc, payoff, index) => acc + payoff * (input.probabilities[index] ?? 0), 0);
...
async function calculate_warrant_irr(input) {
...
async function calculate_conv_note_irr(input) {
